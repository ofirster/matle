...
function getNearestCells(cellRow, cellCol, radius) {
  ...
  for (let i = 1; i <= radius * 2 + 1; i++) {
    if (isCellValid(cell)) res.push(cell);
...
export function getGame() {
  ...
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      ...
      if (piece === "wk") whiteKingCell = [row, col];
      else if (piece === "bk") blackKingCell = [row, col];
    }
  }
  ...
  for (let radius = 1; radius <= 2; radius++) {
    let nearWhiteKing = getNearestCells(
      whiteKingCell[0],
      whiteKingCell[1],
      radius
    );
  ...
  for (let radius = 1; radius <= 2; radius++) {
    let nearBlackKing = getNearestCells(
      blackKingCell[0],
      blackKingCell[1],
      radius
    );
...
export function getGame2() {
  ...
  function generateHiddenSquares(board) {
    ...
    while (hiddenSquares.length < 5) {
      ...
      if (
        shouldSelectKing() &&
        (board[row][col] === "wk" || board[row][col] === "bk")
      ) {
